...
// Utility function to load stocks from stocks.json
const loadStocks = async () => {
  try {
    const data = await fs.readFile("./stocks.json", "utf8");
    console.log("Data: ", data);
    return JSON.parse(data); // Expecting an array of stock symbols
  } catch (error) {
...
// Utility function to fetch stock prices using yahoo-finance2
const fetchStockPrices = async (stocks) => {
  try {
    // yahooFinance.quote accepts comma-separated symbols.
    const quotes = await yahooFinance.quote(stocks.join(","));
    // If only one symbol is requested, yahooFinance.quote returns an object.
    ...
    logger.error("Error fetching stock prices from Yahoo Finance", error);
    return [];
  }
};

// Controller to get Top 5 Gainers
export const getTopGainers = async (req, res) => {
  try {
    const stocks = await loadStocks();
    if (!stocks.length) throw new Error("No stocks found in JSON file.");

    const stockPrices = await fetchStockPrices(stocks);
    // Sort descending by changePercent for gainers
    ...
    return res.status(statusCode.OK).json({
      statusCode: statusCode.OK,
      message: message.stockTopGainers,
      data: sortedGainers,
    });
  } catch (error) {
...
// Controller to get Top 5 Losers
export const getTopLosers = async (req, res) => {
  try {
    ...
    const stockPrices = await fetchStockPrices(stocks);
    // Sort ascending by changePercent for losers
