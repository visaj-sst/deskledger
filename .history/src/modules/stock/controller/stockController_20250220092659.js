...
const loadStocks = async () => {
  console.log("Inside load stocks !! ");
  try {
    const data = await fs.readFile("./stocks", "utf8");
...
const fetchStockPrices = async (stocks) => {
  try {
    console.log("Inside fetch stock prices !! ");
...
export const getTopGainers = async (req, res) => {
  console.log("Inside top gainers ");
  try {
    const stocks = await loadStocks();

    console.log("Stocks Loading : ", stocks);
