...
const fetchStockPrices = async (stocks) => {
  try {
    // yahoo-finance2 can accept a comma separated list of symbols via the quote endpoint.
    // However, to be safe, we'll fetch quotes for all stocks in one call.
    const quotes = await yahooFinance.quote(stocks.join(","));

    // When a single symbol is provided, yahoo-finance2 returns an object.
    // If multiple symbols, it returns an array.
...
export const getTopGainers = async (req, res) => {
  try {
    ...
    return res.status(statusCode.OK).json({
      statusCode: statusCode.OK,
      message: "Top 5 gainers fetched successfully.",
      data: sortedGainers,
    });
  } catch (error) {
...
export const getTopLosers = async (req, res) => {
  try {
    ...
    return res.status(statusCode.OK).json({
      statusCode: statusCode.OK,
      message: "Top 5 losers fetched successfully.",
      data: sortedLosers,
    });
  } catch (error) {
