...
const loadStocks = async () => {
  try {
    ...
    return JSON.parse(data); // Expecting an array of symbols
  } catch (error) {
...
// Fetch stock prices for provided symbols
const fetchStockPrices = async (stocks) => {
  try {
    const symbolString = stocks.join(",");
    console.log("Fetching quotes for: ", symbolString);

    const quotes = await yahooFinance.quote(symbolString);
    console.log("Full API Response: ", quotes);

    // Ensure quotes is an array (if only one symbol is passed, wrap it in an array)
...
export const getTopGainers = async (req, res) => {
  try {
    ...
    // Sort descending by changePercent (largest gainers first)
...
export const getTopLosers = async (req, res) => {
  try {
    ...
    const stockPrices = await fetchStockPrices(stocks);
    // Sort ascending by changePercent (most negative first)
