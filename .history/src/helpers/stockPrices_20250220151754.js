...
export const fetchStockPricesBatch = async (symbolsBatch) => {
  try {
    ...
    return formattedQuotes
      .filter((stock) => stock && stock.symbol) // Ensure valid stocks
      .map((stock) => ({
        symbol: stock.symbol,
...
export const fetchStockPrices = async () => {
  try {
    ...
    if (!stocks.length) {
      throw new Error(
        "No stocks available to fetch. Please check stocks.json."
      );
    }

    const stockSymbols = stocks.map((stock) => stock.symbol).filter(Boolean);

    if (!stockSymbols.length) {
      throw new Error("Stock symbol list is empty after filtering.");
    }
    ...
    if (!allStockPrices.length) {
      throw new Error("No stock data returned from Yahoo Finance API.");
    }
